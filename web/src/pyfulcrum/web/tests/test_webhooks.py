...
class WebhookTestCase(WebTestCase):
    ...
    def test_webhook_errors(self):
        ...
        resp = self._test_client.post('/webhook/test/',
                                      ...
                                      content_type='application/json')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(b'cannot handle some.invalid event type',
                         resp.data, resp.data)

        resp = self._test_client.post('/webhook/test/',
                                      data='{"type": "form.create"}',
                                      content_type='application/json')
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self._test_client.post('/webhook/invalid/',
                                      data='{"type": "form.create",'
                                           '"data": {"id":'
                                           '"7a0c3378-b63a-4707-b459-'
                                           'df499698f23c"}}',
                                      content_type='application/json')
        self.assertEqual(resp.status_code, 404, resp.data)
        self.assertTrue(b'webhook invalid configuration not found'
                        in resp.data, resp.data)
    ...
    def test_webhook_processing(self):
        ...
        form = self.api_manager.forms.get("7a0c3378-b63a-4707-b459-"
                                          "df499698f23c",
                                          cached=True)
